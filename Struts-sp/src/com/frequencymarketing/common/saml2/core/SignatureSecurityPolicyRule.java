...
*/
public class SignatureSecurityPolicyRule  implements  SecurityPolicyRule {
	...
	public SignatureSecurityPolicyRule(CredentialResolver credentialResolver, SAMLSignatureProfileValidator samlSignatureProfileValidator,CriteriaSet criteriaSet) {
		super();
	...
	private void checkMessageSignature(MessageContext messageContext,
		...
		criteriaSet.add( new EntityIDCriteria("usm"));
		...
		try {
			if (!trustEngine.validate( samlMessage.getSignature(), this.criteriaSet)) {
				throw new SecurityPolicyException("Signature was either invalid or signing key could not be established as trusted");
			}
		} catch (SecurityException se) {
			throw new SecurityPolicyException("Error evaluating the signature",se);
